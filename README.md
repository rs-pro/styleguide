...
* Разумно используй z-index. В рамках одного проекта, блоки разделяемые между страницами, должны иметь разные группы z-index. Старайся выделять 10 уровней z-index на каждый такой блок, например от 20 до 30, и внутри блока использовать только z-index из этого диапазона.
